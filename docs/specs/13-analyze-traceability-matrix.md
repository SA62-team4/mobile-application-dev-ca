...
| --- | --- | --- | --- | --- |
| REQ-01 | Android app uses Kotlin with XML layouts | `04-plan-system-architecture.md`, `07-plan-android-ui-flows.md` | `android-app/` screens and layouts, Figma UI spec | Android build, manual UI demo, Figma-to-XML review, confirm AppCompatActivity + View Binding + explicit-Intent navigation + ListView/ArrayAdapter idiom (T-701)|
...
| REQ-04 | User can create wellness records | `05-plan-backend-data-model-erd.md`, `06-plan-api-contracts.md`, `07-plan-android-ui-flows.md` | Create API, form screen with predefined exercise-type spinner, MySQL row, Figma add/edit frame | Backend test, Android manual QA |
...
| REQ-06 | User can update wellness records | `06-plan-api-contracts.md`, `07-plan-android-ui-flows.md` | Update API, edit screen with predefined exercise-type spinner, Figma add/edit frame | Backend test, demo edit |
...
| REQ-10 | Chatbot accepts questions from Android through backend | `06-plan-api-contracts.md`, `07-plan-android-ui-flows.md`, `08-plan-rag-ai-design.md` | Chat endpoint and Android chat screen, Figma chatbot frame, streaming bubble, optional coarse-location fields for backend-mediated premium weather routing, and local-AI waiting/progress states | Integration test, demo question |
| REQ-11 | Chatbot uses basic local RAG | `08-plan-rag-ai-design.md` | KB files including BMI/body metrics guidance, Chroma index, retrieval code | RAG smoke test, source snippets |
| REQ-12 | LLM is free/local only | `04-plan-system-architecture.md`, `08-plan-rag-ai-design.md`, `10-plan-docker-devops.md` | Ollama configuration, optional local premium weather agent, no paid API client | Config review, demo Ollama models |
| REQ-13 | Python agentic AI retrieves data, analyses trends, generates and saves recommendations | `09-plan-agentic-ai-workflow.md`, `07-plan-android-ui-flows.md` | Python agent workflow, backend internal APIs, Figma recommendation and local AI waiting frames; optional Android local broadcast notification for newly saved insights | Agent tests, demo recommendation; optional notification manual QA |
| REQ-14 | Dockerise backend/runtime services where practical | `04-plan-system-architecture.md`, `10-plan-docker-devops.md` | Compose services and volumes; optional backup Compose override for `.NET Backup API`; optional premium weather-agent Compose file | Docker smoke test; optional backup health check on port `8082`; optional premium-agent health check |
...
| S-02 | Android body metrics dashboard | `03-clarify-decisions-and-edge-cases.md`, `05-plan-backend-data-model-erd.md`, `06-plan-api-contracts.md`, `07-plan-android-ui-flows.md` | Weight stored with wellness records, height stored in account profile, dashboard BMI summary with BMI trend sparkline, weight trend, and interactive value tooltips on all sparklines | Dashboard helper tests (incl. BMI series), profile endpoint tests, manual compact-screen QA |
| S-04 | Security hardening: login throttling/lockout, token-expiry UX, no clear-text HTTP outside demo builds | `03-clarify-decisions-and-edge-cases.md` (DEC-016), `06-plan-api-contracts.md`, `07-plan-android-ui-flows.md`, `10-plan-docker-devops.md`, `12-tasks-implementation-backlog.md` | Spring `LoginAttemptService` and its .NET twin, `429` + `Retry-After` from `ApiException`/`GlobalExceptionHandler` and `ApiErrorMiddleware`, Android `LoginLockout` banner and `LoginActivity.redirectIntent(sessionExpired)`, Android `release` build type on the strict `network_security_config.xml` | `LoginAttemptServiceTests` (.NET) covers lock-after-threshold, window expiry, and success-clears-counter; `LoginLockoutTest` (Android) covers message formatting; manual QA of the lockout and session-expired banners; Codex Security diff scan. **Gap: Spring's `LoginAttemptService` has no equivalent unit test yet (T-1004).** |
