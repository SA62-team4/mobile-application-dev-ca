...
| REQ-16 | GitHub collaboration, CI, and deployment are defined | `10-plan-docker-devops.md`, `11-plan-implementation-roadmap.md`, `SECURITY.md` | Branch rules, PR template, CI workflows (incl. `ansible` syntax/lint job), deploy/infra workflows (`deploy.yml` running `ansible-playbook`, `infra.yml`), `docker-compose.prod.yml`, `Caddyfile`, `infra/terraform/`, `infra/ansible/` (bootstrap + app roles), Codex Security scan evidence | Actions pass, PR review, Terraform apply + Ansible deploy from Actions with HTTPS health check, `ansible-playbook --syntax-check` + `ansible-lint` green, Codex Security scan summary |
